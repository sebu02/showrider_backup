	...
	$.widget("ui.formwizard", {
		...
		_init: function() {
			...
			if(this.options.showSteps) {
				function setCurrentStep(curStep, _this) {
					...
					for(var i= 0; i < num; i++ ) {
						if(activatedSteps[i] != curStep ) {
							...
							wstep.each(function( index ) {
								if($(this).attr('data-step-num') == dataStepNum-1) {
									$(this).addClass('done')
									console.log($(this).find('.donut').html('<span class="icon16 iconic-icon-checkmark"></span>'))
								}
							});
						}
					}
				...
				function showSteps(wizsteps, steps, stepNum) {
					for(var i = 0; i < stepNum; i++) {
						stepText = steps[i].getAttribute('data-text');
						stepNumber = steps[i].getAttribute('data-num');
						if(i+1 == stepNum) {
							wizsteps.append(
								'<div class="wstep last-child" data-step-num="'+i+'"><div class="donut">'+ stepNumber+'</div><span class="txt">'+stepText+'</span></div>'
							);
						} else {
							wizsteps.append(
								'<div class="wstep" data-step-num="'+i+'"><div class="donut">'+ stepNumber+'</div><span class="txt">'+stepText+'</span></div>'
							);
						}
					}
				}
				...
				var _steps = _step.find("span[data-text]");
				setDataAttr(_step);
				_steps.hide();
				//vizualize steps in container
				showSteps(this.element.find(".wizard-steps"),_steps, _steps.length);
				//get wizard state
				_state = this._state();
				setCurrentStep(_state.currentStep, this.element);

				this.element.bind("step_shown", function(event, data){
					removeCurrentStep(this);
					setCurrentStep(data.currentStep, this);
