...
//		$CI =& get_instance();
	 ...
	 */	
	function go_upload($field,$my_product) {
        $CI =& get_instance(); 
...
//            echo "$fname\n\n<br><br>\n\n";
		  ...
		  /*$img=  $this->randomvalue().'.'.$_FILES[$field]['name'];
		// echo "<br>";
		 //$_FILES[$field]['name']
		 $fn=$_FILES[$field]['name'][$i];
		 $dir = substr(strrchr($fn, "."), 1);
		
		  $img1=explode('.',$img.$i);
		//  print_r($img1);
		 //echo "<br>";
		    $CI->upload->file_name = 'beauty'.$img1[0].'.'.$dir;*/
			
			 //$CI->upload->file_name = 'lilaq-boutique-'.$my_product.'-'.$this->randomvalue().'.'.end(explode('.',$_FILES[$field]['name'][$i]));
			 
			 $sbn=strtolower($_FILES[$field]['name'][$i]);
			 $file_split_array = explode('.',$sbn);
			 $randomvalue = $this->randomvalue();
			 $CI->upload->file_name = $my_product.'-'.$randomvalue.'.'.end($file_split_array);
            ...
            $CI->upload->file_name = $this->clean_file_name($CI->upload->file_name);
